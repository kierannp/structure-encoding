...
def load_mol_data():
    def to_adj(x):
        try:
            ...
            return Chem.rdmolops.GetAdjacencyMatrix(m)
        except:
            return np.NAN
    data = pd.read_csv('~/datasets/BBBP.csv')
    data['adj'] = data['smiles'].apply(to_adj)
    data = data.dropna()
    graph_list = data['adj'].to_numpy()
    graph_size = [i.shape[0] for i in graph_list]
    return graph_list, graph_size
